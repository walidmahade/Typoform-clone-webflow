...
})({"9BD0Q":[function(require,module,exports) {
...
$(function() {
   ...
   */ const lvFormSlider = new Swiper(".swiper", {
    ...
    function removeErrorMessage($target) {
        ...
        // show ok button
        $targetSectionBtnWrapper.find(".lv-form_ok-btn").first().css({
            opacity: 1
        });
        // hide errors
    ...
    function goToSlideConditionally(conditions, inputVal) {
        ...
        if (newSlideSerial) {
            ...
            if (slideSelector.length) console.log("SLIDE ALREADY EXISTS, NO NEED TO PUSH NEW");
            else {
    ...
    function handleScroll(event) {
        ...
        if (!scrolling) {
            ...
            if (direction === "up") // if direction is up, no need to check for errors
            goToPrevSlide();
            else {
   ...
   */ const multipleChoicesWrappers = $(".lv-form_multichoice");
    ...
    if (multipleChoicesWrappers.length) multipleChoicesWrappers.each(function() {
        const $this = $(this);
        const optionsArr = $this.attr("data-options").split("\n");
        let optionsHtml = "";
        if (optionsArr.length) optionsArr.map(function(opStr, num) {
            optionsHtml += `
...
},{}]},["9BD0Q","igcvL"], "igcvL", "parcelRequire2330")
